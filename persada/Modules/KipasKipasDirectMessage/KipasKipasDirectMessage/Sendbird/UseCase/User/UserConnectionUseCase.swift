...
public class UserConnectionUseCase {
    ...
    public func login(userId: String,
        ...
        SendbirdChat.connect(userId: userId) { [weak self] user, error in
            ...
            guard let user = user else {
                completion(.failure(SBError(domain: "User not found..", code: 404)))
                return
            }
    ...
    public func updateUserInfo(nickname: String?, profileImageURL: String?, completion: @escaping (Result<Void, SBError>) -> Void) {
        let params = UserUpdateParams()
