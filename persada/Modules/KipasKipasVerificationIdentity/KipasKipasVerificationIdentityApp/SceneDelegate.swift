...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    private(set) lazy var scheduler: AnyDispatchQueueScheduler = DispatchQueue(
        label: "com.koanba.kipaskipas.KipasKipasVerificationIdentityApp.queue",
        qos: .userInitiated,
    ...
    private(set) lazy var toolsURL: URL = {
    ...
    private(set) lazy var verifyIdentityStore: VerifyIdentityStoredKeychainStore = {
    ...
    private(set) lazy var verifyIdentityChecker: VerifyIdentityChecker = { [self] in
        let loader = RemoteVerifyIdentityChecker(baseURL: baseURL, client: authenticatedHTTPClient)
        return MainQueueDispatchDecorator(decoratee: loader)
    }()
    
    private(set) lazy var statusLoader: VerifyIdentityStatusLoader = { [self] in
        let loader = RemoteVerifyIdentityStatusLoader(baseURL: toolsURL, client: httpClient)
        return MainQueueDispatchDecorator(decoratee: loader)
    }()
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let client = makeHTTPClient(baseURL: baseURL)
    ...
    func configureRootViewController(in window: UIWindow) {
        ...
        let vc = makeVerifyIdentityController()
        window.rootViewController = UINavigationController(rootViewController: vc)
    }
    
    private func makeVerifyIdentityController() -> VerifyIdentityController {
        let router = VerifyIdentityRouter(handleBackToBalanceView: nil)
        let viewModel = VerifyIdentityViewModel(checker: verifyIdentityChecker)
        let vc = VerifyIdentityController(viewModel: viewModel, verificationStatus: .revision)
        viewModel.delegate = vc
        router.controller = vc
        vc.router = router
        router.handleBackToBalanceView = { [weak self] in
            guard self != nil else { return }
            self?.window?.topViewController?.popToViewController(ofType: VerifyIdentityController.self)
        }
        return vc
    }
...
private extension UIDevice {
    static let deviceDidShakeNotification = Notification.Name(rawValue: "deviceDidShakeVerificationIdentity")
}
