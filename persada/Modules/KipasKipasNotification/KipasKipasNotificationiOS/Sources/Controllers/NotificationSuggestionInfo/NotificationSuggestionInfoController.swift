//
//  NotificationSuggestionInfoController.swift
//  KipasKipasNotificationiOS
//
//  Created by DENAZMI on 03/05/24.
//

import UIKit
import KipasKipasShared

class NotificationSuggestionInfoController: CustomHalfViewController {

    let mainView: NotificationSuggestionInfoView
        
    init() {
        mainView = NotificationSuggestionInfoView()
        super.init(nibName: nil, bundle: nil)
        mainView.delegate = self
    }
    
    required init?(coder: NSCoder) {
        fatalError("init(coder:) has not been implemented")
    }
    
    override func loadView() {
        super.loadView()
        overrideUserInterfaceStyle = .light
        canSlideUp = false
        viewHeight = 240
    }
    
    override func viewDidLoad() {
        super.viewDidLoad()
    }
    
    override func setupConstraints() {
        super.setupConstraints()
        containerView.addSubview(mainView)
        mainView.anchors.leading.equal(containerView.anchors.leading)
        mainView.anchors.trailing.equal(containerView.anchors.trailing)
        mainView.anchors.top.equal(containerView.anchors.top)
        mainView.anchors.bottom.equal(containerView.safeAreaLayoutGuide.anchors.bottom)
    }
}

extension NotificationSuggestionInfoController: NotificationSuggestionInfoViewDelegate {
    func didClose() {
        animateDismissView()
    }
}
