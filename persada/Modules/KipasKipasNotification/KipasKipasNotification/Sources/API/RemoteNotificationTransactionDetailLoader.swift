//
//  RemoteNotificationTransactionDetailLoader.swift
...
public class RemoteNotificationTransactionDetailLoader: NotificationTransactionDetailLoader {
    
    private let url: URL
    private let client: HTTPClient
    public typealias Result = NotificationTransactionDetailLoader.ResultTransaction
    ...
    public func load(request: NotificationTransactionDetailRequest, completion: @escaping (Result) -> Void) {
        
        let urlRequest = NotificationEndpoint.transactionDetail(request: request).url(baseURL: url)
        
        Task {
            do {
                let (data, response) = try await client.request(from: urlRequest)
                completion(RemoteNotificationTransactionDetailLoader.map(data, from: response))
            } catch {
    ...
    private static func map(_ data: Data, from response: HTTPURLResponse) -> NotificationTransactionDetailLoader.ResultTransaction {
        do {
            let items = try NotificationTransactionDetailItemMapper.map(data, from: response)
...
public class NotificationTransactionDetailItemMapper {
        ...
        var item: NotificationTransactionDetailItem? {
             let data = data.map({
                ...
                let currency = NotificationTransactionItemCurrency(orderId: $0.currency?.orderId ?? "", withdrawCurrencyId: $0.currency?.withdrawCurrencyId ?? "", currencyType: $0.currency?.currencyType ?? "", qty: $0.currency?.qty ?? 0, price: $0.currency?.price ?? 0 , totalWithdraw: $0.currency?.totalWithdraw ?? 0, bankAccount: $0.currency?.bankAccount ?? "", storeName: $0.currency?.storeName ?? "", bankAccountName: $0.currency?.bankAccountName ?? "", bankName: $0.currency?.bankName ?? "", bankFee: $0.currency?.bankFee ?? 0, modifyAt: $0.currency?.modifyAt ?? "", status: $0.currency?.status ?? "", noInvoice: $0.currency?.noInvoice ?? "")
                 ...
                 return NotificationTransactionDetailItem(
                    id: $0.id ?? "",
    ...
    static func map(_ data: Data, from response: HTTPURLResponse) throws -> NotificationTransactionDetailItem {
        guard response.statusCode == 200, let root = try? JSONDecoder().decode(Root.self, from: data) else {
            throw KKNetworkError.invalidData
        }
        
        guard let item = root.item else {
            throw KKNetworkError.invalidData
        }
