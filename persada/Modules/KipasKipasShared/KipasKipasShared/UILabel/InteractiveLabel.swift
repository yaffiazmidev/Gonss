...
public protocol ActiveLabelDelegate: AnyObject {
    func didSelect(_ text: String, type: InteractiveLabelType)
}

public typealias ConfigureLinkAttribute = (InteractiveLabelType, [NSAttributedString.Key : Any], Bool) -> ([NSAttributedString.Key : Any])
typealias ElementTuple = (range: NSRange, element: InteractiveLabelElement, type: InteractiveLabelType)

@IBDesignable open class InteractiveLabel: UILabel {
    
    // MARK: - public properties
    open weak var delegate: ActiveLabelDelegate?
    
    open var enabledTypes: [InteractiveLabelType] = [.mention, .hashtag, .url]
    ...
    open var customColor: [InteractiveLabelType : UIColor] = [:] {
        didSet { updateTextStorage(parseText: false) }
    }
    open var customSelectedColor: [InteractiveLabelType : UIColor] = [:] {
        didSet { updateTextStorage(parseText: false) }
    }
    ...
    @IBInspectable public var highlightedFont: UIFont? = nil {
        didSet { updateTextStorage(parseText: false) }
    }
    ...
    open func handleCustomTap(for type: InteractiveLabelType, handler: @escaping (String) -> ()) {
    ...
    open func removeHandle(for type: InteractiveLabelType) {
        switch type {
        case .hashtag:
    ...
    // MARK: - customzation
    @discardableResult
    open func customize(_ block: (_ label: InteractiveLabel) -> ()) -> InteractiveLabel {
        _customizing = true
        block(self)
        _customizing = false
        updateTextStorage()
    ...
    internal var customTapHandlers: [InteractiveLabelType : ((String) -> ())] = [:]
    ...
    lazy var activeElements = [InteractiveLabelType: [ElementTuple]]()
    ...
    fileprivate func addLinkAttribute(_ mutAttrString: NSMutableAttributedString) {
        ...
        for (type, elements) in activeElements {
            ...
            if let highlightFont = highlightedFont {
    ...
    fileprivate func parseTextAndExtractActiveElements(_ attrString: NSAttributedString) -> String {
        ...
        if enabledTypes.contains(.url) {
            let tuple = InteractiveLabelBuilder.createURLElements(from: textString, range: textRange, maximumLength: urlMaximumLength)
        ...
        for type in enabledTypes where type != .url {
            ...
            let hashtagElements = InteractiveLabelBuilder.createElements(type: type, from: textString, range: textRange, filterPredicate: filter)
    ...
    fileprivate func didTap(_ element: String, for type: InteractiveLabelType) {
...
extension InteractiveLabel: UIGestureRecognizerDelegate {
