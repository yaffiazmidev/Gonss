...
open class PagingViewController:
    ...
    /// Determine if the child view controller (content) in page view controller
    /// should be animated or not when selected
    /// _Default: true
    public var animateContentOnSelect: Bool {
        get { return options.animateContentOnSelect }
        set { options.animateContentOnSelect = newValue }
    }
    ...
    open func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        pagingController.select(indexPath: indexPath, animated: options.animateContentOnSelect)
    }
