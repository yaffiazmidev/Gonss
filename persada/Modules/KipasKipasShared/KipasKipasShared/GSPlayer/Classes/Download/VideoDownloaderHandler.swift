...
protocol VideoDownloaderHandlerDelegate: AnyObject {
    ...
    func handler(_ handler: VideoDownloaderHandler, didReceive data: Data, currentOffset: Int, isLocal: Bool)
...
extension VideoDownloaderHandler: VideoDownloaderSessionDelegateHandlerDelegate {
    ...
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        if cacheHandler.cache(data: data, for: range)
        {
            ...
            delegate?.handler(self, didReceive: data, currentOffset: startOffset, isLocal: false)
...
private extension VideoDownloaderHandler {
    ...
    func processActions() {
        ...
        guard action.actionType == .remote else {
            ...
            delegate?.handler(self, didReceive: data, currentOffset: startOffset, isLocal: true)
            processActions()
            return
        }
