...
open class VideoPlayerView: UIView {
    ...
    public static let shouldPausedWhenNotVisible = Notification.Name("shouldPausedWhenNotVisible")
    public static let shouldResumeWhenVisible = Notification.Name("shouldResumeWhenVisible")
    ...
    public enum State {
        ...
        /// Paused, will be called repeatedly when the buffer progress changes
        case buffer(playProgress: Double, bufferProgress: Double)
    ...
    public enum PausedReason: Int {
        ...
        /// The video player is covered (e.g. by modal)
        case covered
    }
    ...
    public var shouldPlayAfterPreloadFinished: Bool = true
    
    private var isLoaded = false
    private var isReplay = false
    
    private var scheduler: AnyStopwatch?
...
@objc extension VideoPlayerView {
    ...
    open func play(for url: URL) {
        ...
        if shouldPlayAfterPreloadFinished {
            startScheduler(for: url)
        } else {
            setPlayerItem()
        }
    }
    
    private func startScheduler(for url: URL) {
        state = .loading
        SingleVideoPreloadManager.shared.start(for: url)
        
        scheduler = StopwatchPublisher(interval: 0.5)
            .handleEvents(receiveOutput: { [weak self] _ in
                guard let self = self else { return }
                if SingleVideoPreloadManager.shared.isFinished && state != .pausedOnce {
                    stopScheduler()
                    setPlayerItem()
                }
            })
...
private extension VideoPlayerView {
    ...
    private func configureNotificationObservers() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(handleApplicationWillEnterForeground(_:)), name: .AVPlayerItemPlaybackStalled, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(shouldPauseVideoWhenNotVisible), name: VideoPlayerView.shouldPausedWhenNotVisible, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(shouldResumeVideoWhenVisible), name: VideoPlayerView.shouldResumeWhenVisible, object: nil)
    }
             
    @objc private func shouldPauseVideoWhenNotVisible(_ aNotification: Notification) {
        state = .pausedOnce
        pause(reason: .covered)
    }
    
    @objc private func shouldResumeVideoWhenVisible(_ aNotification: Notification) {
        state = .resume
        resume()
    }
    
    @objc private func handleApplicationWillResignActive(_ aNotification: Notification) {
        if pausedReason != .covered && playbackPausesWhenResigningActive {
            state = .pausedOnce
            pause(reason: .hidden)
        }
    }
    ...
    @objc private func handleApplicationDidBecomeActive(_ aNotification: Notification) {
        if pausedReason != .covered && playbackResumesWhenBecameActive {
            state = .resume
            resume()
        }
    }
    ...
    @objc private func handleApplicationDidEnterBackground(_ aNotification: Notification) {
        if pausedReason != .covered && playbackPausesWhenBackgrounded {
            state = .pausedOnce
            pause(reason: .hidden)
        }
    }
    ...
    @objc private func handleApplicationWillEnterForeground(_ aNoticiation: Notification) {
        if pausedReason != .covered && playbackResumesWhenEnteringForeground {
            state = .resume
            resume()
        }
    }
    ...
    func observe(player: AVPlayer?) {
        ...
        playerTimeControlStatusObservation = player.observe(\.timeControlStatus) { [unowned self] player, _ in
            ...
            case .paused:
                ...
                self.state = .buffer(playProgress: self.playProgress, bufferProgress: self.bufferProgress)
    ...
    func observe(playerItem: AVPlayerItem?) {
        ...
        playerBufferingObservation = playerItem.observe(\.loadedTimeRanges) { [unowned self] item, _ in
            if case .buffer = self.state, self.pausedReason != .hidden && self.pausedReason != .covered {
                self.state = .buffer(playProgress: self.playProgress, bufferProgress: self.bufferProgress)
            }
        }
...
extension VideoPlayerView.State: Equatable {
    ...
    public static func == (lhs: VideoPlayerView.State, rhs: VideoPlayerView.State) -> Bool {
        ...
        case (.playing, .playing):
            return true
        case let (.buffer(p1, b1), .buffer(p2, b2)):
            return (p1 == p2) && (b1 == b2)
        case let (.error(e1), .error(e2)):
            return e1 == e2
        case (.pausedOnce, .pausedOnce):
            return true
        case (.resume, .resume):
            return true
        default:
...
public struct VideoPlayerManager {
    public static func destroy() {
        SingleVideoPreloadManager.shared.destroy()
        VideoPreloadManager.shared.destroy()
        VideoPreloadManager.nullify()
        VideoLoadManager.nullify()
    }
}
