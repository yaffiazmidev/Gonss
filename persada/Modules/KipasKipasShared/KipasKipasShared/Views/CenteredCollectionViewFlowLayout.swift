...
/// Docs: https://github.com/BenEmdon/CenteredCollectionView/blob/master/USAGE.md
...
open class CenteredCollectionViewFlowLayout: UICollectionViewFlowLayout {
    ...
    override open func invalidateLayout(with context: UICollectionViewLayoutInvalidationContext) {
        ...
        if !currentCollectionViewSize.equalTo(lastCollectionViewSize) || lastScrollDirection != scrollDirection || lastItemSize != itemSize {
            ...
            case .vertical:
                ...
                let yContentOffset = (collectionView.contentSize.height / 2) - (collectionView.bounds.size.height / 2)
                collectionView.contentOffset = CGPoint(x: 0, y: yContentOffset + itemSize.height)
            default:
    ...
    open override func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        let itemSize = (collectionView!.delegate as? UICollectionViewDelegateFlowLayout)?.collectionView?(collectionView!, layout: self, sizeForItemAt: indexPath) ?? .zero
    ...
    open override func layoutAttributesForSupplementaryView(ofKind elementKind: String, at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        if elementKind == UICollectionView.elementKindSectionHeader {
            ...
            attributes.frame = CGRect(x: 0, y: collectionView!.bounds.minY, width: collectionView!.bounds.width, height: headerHeight)
            return attributes
        }
        return nil
    }
