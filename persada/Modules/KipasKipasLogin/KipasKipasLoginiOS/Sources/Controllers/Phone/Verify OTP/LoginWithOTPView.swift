...
final class LoginWithOTPView: UIView {
    ...
    private let switchToCallButton = KKBaseButton()
    ...
    private let callMethodStackView = UIStackView()
    
    private var cancellables = Set<AnyCancellable>()
    
    var didFinishInputOTP: Closure<String>?
    var didTapSwitchOTPToCall: (() -> Void)?
    ...
    private func observe() {
        ...
        switchToCallButton.tapPublisher
            .sink { [weak self] _ in
                self?.didTapSwitchOTPToCall?()
            }
            .store(in: &cancellables)
    }
}
...
extension LoginWithOTPView {
    ...
    func setTimerText(
        ...
        /// Indicate the logic only processed once
        if callMethodStackView.isHidden {
            callMethodStackView.isHidden = shouldShowCallMethod == false
        }
    }
    
    func showRequestedTooManyOTPError() {
        resendOTPStackView.isHidden = true
    ...
    func setSubheadingText(_ phone: Phone) {
        subheadingLabel.text = "Kode akan dikirimkan ke +62 \(phone)"
    }
}

// MARK: UI
private extension LoginWithOTPView {
    func configureUI() {
    ...
    func configureHeadingLabel() {
        headingLabel.text = "Masukkan kode 4 digit"
    ...
    /*
    func configureCallMethodStackView() {
        let wordingLabel = UILabel()
        wordingLabel.text = "Tidak mendapatkan kode?"
        wordingLabel.textColor = .night
        wordingLabel.font = .roboto(.regular, size: 14)
        wordingLabel.textAlignment = .left
        
        switchToCallButton.setBackgroundColor(.clear, for: .disabled)
        switchToCallButton.setTitle("Minta panggilan telepon", for: .normal)
        switchToCallButton.setTitleColor(.watermelon, for: .normal)
        switchToCallButton.titleLabel?.font = .roboto(.regular, size: 14)
        switchToCallButton.contentHorizontalAlignment = .leading
        
        callMethodStackView.alignment = .center
        callMethodStackView.distribution = .fill
        callMethodStackView.spacing = 8
        callMethodStackView.isHidden = true
        
        callMethodStackView.addArrangedSubview(wordingLabel)
        callMethodStackView.addArrangedSubview(switchToCallButton)
        callMethodStackView.addArrangedSubview(invisibleView())
        
        stackView.addArrangedSubview(callMethodStackView)
    }
     */
