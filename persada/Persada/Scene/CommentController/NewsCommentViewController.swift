...
class NewsCommentViewController : UIViewController, ListAdapterDataSource, UIGestureRecognizerDelegate, AlertDisplayer {
...
//        self.navigationItem.leftBarButtonItem = backButton
    ...
    @objc func refresh(_ sender: AnyObject) {
        commentDataSource?.getHeaderNewsComment()
    }
    ...
    override func viewDidLoad() {
        ...
        commentDataSource?.newsCommentViewModel.bind { [weak self] newsCommentViewModel in
            DispatchQueue.main.async {
                guard let news = newsCommentViewModel else { return }
        ...
        commentDataSource?.newsCommentUpdated.bind(onNext: { [weak self ] newsCommentViewModel in
            guard let news = newsCommentViewModel else { return }
            guard let self = self else { return }
            self.data = [news]
        ...
        sendCommentButton.onTap {
            ...
            guard let id = self.commentDataSource?.newsCommentViewModel.value?.postID else { return }
    ...
    func listAdapter(_ listAdapter: ListAdapter, sectionControllerFor object: Any) -> ListSectionController {
        return NewsCommentSectionController()
    }
...
extension NewsCommentViewController: ReportFeedDelegate {
    func reported() {
        DispatchQueue.main.async {
...
extension NewsCommentViewController {
    func hideMentions() {
        filter("")
    }
...
extension NewsCommentViewController: UITableViewDelegate {
    ...
    func filter(_ string: String) {
        ...
        commentDataSource?.searchAccount(string)
