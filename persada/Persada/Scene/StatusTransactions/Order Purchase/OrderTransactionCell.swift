//
//  OrderTransactionWithButtonCell.swift
//  KipasKipas
//
...
protocol OrderTransactionCell : UITableViewCell {
	var item: OrderCellViewModel! { get }
	var handleTransaction: ((String, String, String?) -> Void)? { get set }
	func setupView()
	func setUpCell(item: OrderCellViewModel, buttonText : String)
	func handleTransactionWhenTappedButton()
}

class OrderTransactionWithButtonCell : UITableViewCell, OrderTransactionCell {
    ...
    func setUpCell(item object: OrderCellViewModel, buttonText : String) {
		...
		priceLabel.text = "\(object.productPrice.toMoney())"
        ...
        case ("COMPLETE", "SETTLED", "DELIVERED", "WAIT"):
            statusLabel.text = "Selesai"
//        case ("CANCELLED", "SETTLED", "DELIVERED"):
//            statusLabel.text = "Dibatalkan"
        default:
            break
        }
...
class OrderTransactionWithoutButtonCell : UITableViewCell, OrderTransactionCell {
	var item: OrderCellViewModel!
	...
	func setUpCell(item object: OrderCellViewModel, buttonText : String) {
        ...
        if object.productType == "RESELLER" {
            ...
            let commission = (object.modal ?? 0) + (object.commission ?? 0)
            
            let attributedText: NSMutableAttributedString = NSMutableAttributedString(string: "\(commission.toMoney())",
