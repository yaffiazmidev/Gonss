...
final class ReportPopupView: UIView {
    ...
    lazy var subtitleLabel: UILabel = {
        let label = UILabel()
        label.translatesAutoresizingMaskIntoConstraints = false
        label.font = .AirBnbCereal(.bold, size: 10)
    ...
    lazy var backButton : PrimaryButton = {
        ...
        button.setup(font: .AirBnbCereal(.bold, size: 14))
        button.isEnabled = false
        button.isUserInteractionEnabled = true
        button.backgroundColor = .primary
        button.isEnabled = true
        button.setTitle(String.get(.backToHome), for: .normal)
    ...
    override init(frame: CGRect) {
        ...
        backButton.anchor(top: subtitleLabel.bottomAnchor, left: nil, bottom: stack.bottomAnchor, right: nil, paddingTop: 24, paddingLeft: 20, paddingRight: 20, width: 214, height: 48)
