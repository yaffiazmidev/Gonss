...
public class MentionListener: NSObject {
    ...
    /**
     @brief Maximum character limit
     */
    private let maxLang: Int
    
    public var handleMaxLangTriggred: (() -> Void)?
...
extension MentionListener: UITextViewDelegate {
    public func textView(_ textView: UITextView, shouldChangeTextIn range: NSRange,
        ...
        // Check if adding the replacement text exceeds the maximum character limit
        if let currentText = textView.text,
           let textRange = Range(range, in: currentText) {
            let updatedText = currentText.replacingCharacters(in: textRange, with: text)
            if updatedText.count > maxLang {
                // Truncate the pasted text if it exceeds the maximum character limit
                let truncatedText = String(updatedText.prefix(maxLang))
                textView.text = truncatedText
                shouldChangeText = false
                self.handleMaxLangTriggred?()
            }
        }
