...
open class TabView: UIScrollView {
    ...
    open override func safeAreaInsetsDidChange() {
        ...
        if options.style == .segmented {
            width.constant = options.margin * -2 - safeAreaInsets.left - safeAreaInsets.right
        }
...
//					tabItemView.setCounter(value: itemCount)
            ...
            case .flexible:
                if options.needsAdjustItemViewWidth {
                    ...
                    adjustCellSize.width = tabItemView.titleLabel.intrinsicContentSize.width + 70.0
    ...
    private func layout(containerView: UIView, containerWidth: CGFloat) {
        ...
        case .segmented:
            if #available(iOS 11.0, *), options.isSafeAreaEnabled {
                leftMarginConstraint = containerView.leadingAnchor.constraint(equalTo: self.leadingAnchor, constant: options.margin + safeAreaInsets.left)
                width = containerView.widthAnchor.constraint(equalTo: self.widthAnchor, constant: options.margin * -2 - safeAreaInsets.left - safeAreaInsets.right)
                NSLayoutConstraint.activate([
                    containerView.topAnchor.constraint(equalTo: self.topAnchor),
                    leftMarginConstraint,
                    width,
                ...
                leftMarginConstraint = containerView.leadingAnchor.constraint(equalTo: self.leadingAnchor, constant: options.margin)
                width = containerView.widthAnchor.constraint(equalTo: self.widthAnchor, constant: options.margin * -2)
                NSLayoutConstraint.activate([
                    containerView.topAnchor.constraint(equalTo: self.topAnchor),
                    leftMarginConstraint,
                    width,
