//
//  SimpleResourceDelegate.swift
//  KipasKipas
//
//  Created by Zein Rezky Chandra on 07/08/21.
...
class SimpleResourceLoaderDelegate: NSObject, AVAssetResourceLoaderDelegate, URLSessionDataDelegate, URLSessionTaskDelegate {
    ...
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        print("YEY DAPAT DATAA \(url) \(data)")
    ...
    private func saveMediaDataToLocalFile() -> URL? {
        ...
        if FileManager.default.fileExists(atPath: fileURL.path) {
            do {
                print("upssss file nya sudah ada")
        ...
        do {
            print("yeayyy disimpan berhasil ke \(fileURL)")
