...
NSString *const TUIGIFTIM_SIGNALING_KEY_SOURCE = @"source";
NSString *const TUIGIFTIM_SIGNALING_KEY_ADMINISTRATOR = @"administrator";
...
- (void)onRecvGroupCustomMessage:(NSString *)msgID groupID:(NSString *)groupID sender:(V2TIMGroupMemberInfo *)info customData:(NSData *)data {
	if ([self.groupID isEqualToString:groupID] && data) {
        ...
        NSString *source = dic[TUIGIFTIM_SIGNALING_KEY_SOURCE];
		...
		if ([businessID isEqualToString:TUIGIFTIM_DATA_BUSINESSID]
            && [source isEqualToString:TUIGIFTIM_SIGNALING_KEY_ADMINISTRATOR]
            ) {
...
- (NSDictionary *)getDefaultExtInfo {
    ...
    NSDictionary *info = @{@"userName": ([TUILogin getNickName] ?: @""),
